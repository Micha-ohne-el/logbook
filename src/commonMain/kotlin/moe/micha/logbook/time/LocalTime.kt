...
class LocalTime(
	...
	fun formatAsStandard() = localDateTime.toString()
	 ...
	 */
	fun formatWithSimplePattern(pattern: String): String {
		...
		for (symbol in simpleFormat) {
			formatted = formatted.replace(Regex(Regex.escape(symbol.key.toString()) + "+")) { matchResult ->
				...
				val replacement = symbol.getValue(localDateTime, requestedLength)
